...
class GraphScript {
  ...
  private runCliScript() {
    ...
    yargs
      .command(
        "fromfile",
        "generate the graph image from files",
        (yargs) => {
          return yargs
            .option("template", {
        ...
        (argv) => {
          ...
          const template = GraphScript.jsonFileToObject(
            path.join(FILE_PATHS_RELATIVE_TO, argv.template)
          );
          const answerSets = GraphScript.jsonFileToObject(
            path.join(FILE_PATHS_RELATIVE_TO, argv.as)
          );
      ...
      .command(
        "fromstr",
        "generate the graph image from json string inputs",
        (yargs) => {
          return yargs
            .option("template", {
              describe: "the input json template file path",
        ...
        (argv) => {
          ...
          rl.question("", (jsonStr) => {
            ...
            console.log(`Using <<${argv.template}>> as template file...`);
            console.log(
              `Using answer set json from stdin (${jsonStr.length} chars)...`
            );
            console.log(`Using ${argv.output} as output directory...`);
            console.log();
