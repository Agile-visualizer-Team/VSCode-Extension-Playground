...
describe("[table] TableCreator", () => {
    ...
    it('run_scripts has to call create_table_from_atoms_list not more \
        time than the maximum number of as to convert in this case 1', () => { // the single test

        
        const mock_table_creator = sinon.mock(table_creator);
        const methos_mocked=sinon.stub(table_creator, 'maxNumOfAnswerSetToConvert').returns(1);
        const create_image_mock=sinon.stub(table_creator, 'create_image_from_html').returns(true);
    ...
    it('run_scripts has to call create_table_from_atoms_list not more \
        time than the maximum number of as to convert in this case 2', () => { // the single test

        
        const mock_table_creator = sinon.mock(table_creator);
        let method= sinon.stub(table_creator, 'maxNumOfAnswerSetToConvert').returns(3);
    ...
    testForCalls.forEach((test) => {
        ...
        based on the number of config', () => {
            sinon.stub(table_creator, 'maxNumOfAnswerSetToConvert').returns(test.config_value);
            const mock_matrix_creator = sinon.mock(table_creator);
            const create_image_mock=sinon.stub(table_creator, 'create_image_from_html').returns(true);

            mock_matrix_creator.expects('create_table_from_atoms_list').exactly(test.config_value);
...
describe("[table] retrieve_answer_set_as_tuple_array method tests", () => {
    ...
    mapped values', () => { // the single test

        let mapped_atom: string ='cell';
        let as: string[]=['cell(1,2,ciao)', 'cell(0,0,hello)']

        let atoms_splitted_array:string[][]=[]
    
        let obtained_value:boolean = table_creator.retrieve_answer_set_as_tuple_array(as, atoms_splitted_array, mapped_atom);
        let expected_result:boolean = true;
        let expected_0_array:string[]=['1','2', 'ciao']
        let expected_1_array:string[]=['0','0', 'hello']
...
describe("[table] create_html_table_for_mapped_atom method tests", () => {
    ...
    if the internal method retrieve_answer_set_as_tuple_array returns false', () => { // the single test

        let mapped_atom: string ='cell';
        let as: string[]=[]
        let atoms_splitted_array:string[][]=[]
        let table_html = '';
        sinon.stub(table_creator, 'retrieve_answer_set_as_tuple_array').returns(false);

        
        let obtained_value:string = table_creator.create_html_table_for_mapped_atom(as, mapped_atom, table_html);
      

        let expected_table_html:string = '<strong>Problem in table creating, make sure data is correct (check the answer set)</strong>'
    ...
    if the internal method retrieve_answer_set_as_tuple_array returns true but the array is empty', () => { // the single test

        let mapped_atom: string ='cell';
        let as: string[]=[]
        let atoms_splitted_array:string[][]=[]
        let table_html = '';
        sinon.stub(table_creator, 'retrieve_answer_set_as_tuple_array').returns(true);

        
        let obtained_value:string = table_creator.create_html_table_for_mapped_atom(as, mapped_atom, table_html);
      

        let expected_table_html:string = '<strong>No value found in answer set for: ' + mapped_atom + '</strong><br>';
        
        expect(obtained_value).to.eql(expected_table_html);
    });

    it('create_html_table_for_mapped_atom has call create_table_html if  \
    if the internal method retrieve_answer_set_as_tuple_array returns true and the arrays length is more than 0', () => { // the single test

        let mapped_atom: string ='cell';
        let as: string[]=['cell(1,2,ciao)']
        let atoms_splitted_array:string[][]=[]
        let table_html = '';
...
describe("[table] create_table_html method tests", () => {
    ...
    it('create_table_html returns an empty string if atoms_splitted_array is empty', () => { // the single test

        let mapped_atom: string ='cell';
        let atoms_splitted_array:string[][]=[]

        let obtained_table_html:string = table_creator.create_table_html(atoms_splitted_array, mapped_atom);
        let expected_table_html:string = '';
    ...
    it('create_table_html returns an empty string if mapped_atom is empty', () => { // the single test

        let mapped_atom: string ='';
        let atoms_splitted_array:string[][]=[['1','2', 'ciao']]
    ...
    it('create_table_html returns a table if atoms_splitted_array contains the mapped atoms', () => { // the single test
...
describe("[table] create_table_html method tests", () => {
    ...
    it('create_html_to_convert_in_image returns the right string when we pass every table html ', () => { // the single test
