...
export class TableCreator{
     ...
     */
    create_table_from_atoms_list(atoms_list: string[], mapping_list: string[], index: number) {
    ...
    */
    create_table_html(atoms_splitted_array: string[][], mapped_atom: string) {
        if (atoms_splitted_array == undefined || atoms_splitted_array.length == 0) return '';
        if (mapped_atom == undefined || mapped_atom == '') return '';


        let html_table: string = `<table><thead><tr class="titolo"><th>Answer set</th><th>Mapped value: ` + mapped_atom + `</th></tr></thead><tbody><tr>`;
     ...
     */
    retrieve_answer_set_as_tuple_array(atoms_list: string[], atoms_splitted_array: string[][], mapped_atom: string): boolean {
 ...
 */
    read_commands_and_run(){
        ...
        yargs
            .command('fromfile',
                'generate the table images from files', (yargs) => {
                    return yargs
                    .option('template', {
            ...
            .command('fromstr',
                'generate the table images from json string inputs', (yargs) => {
                    return yargs
                    .option('template', {
                        describe: 'the input json template file path',
                        type: 'string',
